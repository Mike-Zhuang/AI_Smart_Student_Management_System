...
“生涯发展与选科”中的公开专业数据已升级为“推荐院校与近三年录取分”。

- 默认按学生最近三次成绩加权，并把语文、数学、英语和当前选科三科折算到 750 分。
- 可切换为最近一次考试、指定考试、多次趋势拟合、全科折算、原始总分或手动输入分数。
...
- `锟斤拷`
- `瀛﹂敓...`
- `鐜嬭开`、`寮犳垐` 这类 UTF-8 内容被 GBK/GB18030 误读后的短姓名乱码
- `�`
- 考试名称异常学年值
...
每次文本修复规则升级时，后端会更新内部维护版本号。线上通过 GitHub 自动同步并重启后，会在后台自动再跑一次历史数据文本维护，不需要手工登录服务器改数据库。

若老师不熟悉 CSV，优先使用系统提供的 XLSX 模板即可。

### 8.6 班级空间图片为什么会被压缩

心灵驿站和班级风采的图片会在上传后压缩为展示用 WebP，最长边限制在 1920px 左右，只保留压缩后的展示图，不保存原图。这样既能在班级空间正常预览照片，又能避免长期占用过多服务器磁盘。

图片访问需要登录并校验班级权限；学生、家长、教师只能看到自己有权访问班级的图片。

当前媒体接口会返回 `Cache-Control`、`ETag` 和 `Last-Modified`，浏览器再次打开同一张图时可以走缓存或 304；前端也会复用已取回的鉴权图片 Blob，并对班级空间首屏图片做轻量预取。图片质量参数不下调，主要通过缓存减少重复下载。

### 8.7 首页待办为什么可以直接进入请假处理

首页“请假待处理”和班主任工作台“当前待办”会跳转到家校沟通的请假进度区域，并自动筛选待处理请假，方便班主任直接审批。

跳转地址为 `/dashboard/home-school?focus=leave&status=pending`，原有请假审批、导出和批量删除按钮位置保持不变。

### 8.8 家长代提交请假为什么会直接进入班主任审批
