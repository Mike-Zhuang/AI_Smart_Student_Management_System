...
并且会在导入、管理列表查询、班级空间、组织架构等接口出口继续做乱码修复与考试名称规范化，尽量避免出现 `锟斤拷`、`�`、异常学年值等问题。

若老师不熟悉 CSV，优先使用系统提供的 XLSX 模板即可。

### 8.6 选科建议流式中断或没有完成事件怎么办

当前版本已经对流式链路做了专门加固：

1. 后端会尽量保证成功时发出 `complete`、失败时发出 `error`
2. 前端会兼容多段 `data:`、不同换行符和“只有最终结果没有明显 delta”的返回
3. 即使模型没有明确完成事件，只要最终结果已齐全，系统也会按成功结果落库并刷新历史
