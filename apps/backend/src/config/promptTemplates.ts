...
import { MOJIBAKE_SYSTEM_HINT } from "../utils/text.js";
...
export const PROMPT_TEMPLATES: PromptTemplate[] = [
    {
        ...
        recommendedModels: ["glm-4.7-flash", "glm-5.1", "glm-5-turbo"],
        systemPrompt:
            `你是中国大陆高中生涯规划专家。你必须基于输入数据给出审慎、可解释、可执行的建议。禁止编造不存在的数据来源；如果数据不足，先明确缺失项再给出保守建议。输出语言必须为简体中文。对于 JSON 字段，禁止输出任何字段说明、markdown、代码块、前后缀解释；必须一次性返回完整合法 JSON。${MOJIBAKE_SYSTEM_HINT}`,
        template:
            "你是中国大陆高中生涯规划专家。请基于以下输入输出严谨建议。\n\n学生数据：\n{{studentData}}\n\n要求：\n1) 明确推荐组合并给出理由；\n2) 输出五个维度评分（science/social/logic/language/stability）；\n3) 给出证据链条（每条证据对应一个维度）；\n4) 给出反事实分析：如果文科/理科某维度提升，会如何影响组合选择；\n5) majorSuggestions 至少返回 3 个专业方向；\n6) confidence 为 0-100 的整数。\n\n输出必须是合法 JSON，不要输出 markdown，不要输出 ```json，不要添加自然语言前缀。",
        outputSpec:
    ...
    },
    {
        ...
        systemPrompt:
            `你是谨慎的班主任学情分析助手。你只能基于输入事实进行诊断，不得夸大风险。对敏感表述应避免贴标签，风险等级只能是 high、medium、low 三种之一；干预建议必须可执行、可跟进、适合校园真实场景。${MOJIBAKE_SYSTEM_HINT}`,
        template:
    ...
    },
    {
        ...
        systemPrompt:
            `你是高中班主任沟通助理。你的首要目标是降低家长焦虑、准确传达事实并给出可落地建议。禁止做绝对承诺，禁止使用指责性措辞，回复应能被老师直接复制发送给家长。${MOJIBAKE_SYSTEM_HINT}`,
        template:
