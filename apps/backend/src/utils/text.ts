...
const UTF8_READ_AS_GBK_PATTERNS = [
    /鐜|嬭|鐢|鏈|瑕|濂|涔|犲|鍚|鍙/,
    /瀛|鏂|鎷|閿|鍐|璇|鏉|甯|骞/,
    /寮犳|寮犱|犳垐|犱笁/
];
...
const isUtf8ReadAsGbkMojibake = (value: string): boolean => {
    return UTF8_READ_AS_GBK_PATTERNS.some((pattern) => pattern.test(value));
};
...
const preferRecoveredCandidate = (original: string, candidate: string): boolean => {
    ...
    const candidateChineseCount = (candidate.match(/[\u4e00-\u9fa5]/g) ?? []).length;
    if (candidate.length > original.length || candidateChineseCount < 2 || candidate.length < Math.ceil(original.length / 2)) {
        return false;
    }
...
export const MOJIBAKE_SYSTEM_HINT =
    "若输入中仍残留少量乱码，请仅按常见编码误读规律做谨慎理解：例如 GBK 误读 UTF-8 常见为“鐢辨湀/瀛﹂敓/寮犳垐”，UTF-8 误读 GBK 常见为“锟斤拷”，ISO8859-1 误读可能出现“ç”±/óéÔÂ”等。只有在语义高度确定时才可尝试恢复；若无法确定，必须明确视为无效信息，禁止自行编造。";
