...
- 点击“请假待处理”会直接进入“家校沟通 → 请假进度”，并自动筛选待处理请假。
...
- 学生提交请假：仍需家长确认后再进入班主任审批
- 家长代提交请假：系统会直接跳过“家长确认”，时间线会显示“家长已代提交，进入班主任审批”
...
- “推荐院校与近三年录取分”会根据学生当前成绩画像推荐高校专业，并展示已有年份录取分、折算分差和冲刺/匹配/保底层级
...
- GET /api/career/major-recommendations?studentId=...
...
1. 查看当前班级待办漏斗与重点关注学生
2. 新增成长记录 / 班级日志
3. 发布心灵驿站内容并支持附件上传与删除，图片会自动压缩后用于班级空间预览
4. 记录小组积分并查看排行榜
5. 上传班级风采照片并维护班级简介

操作提示：

- “当前待办”中的请假待处理项可点击进入家校沟通页面，系统会自动定位到待处理请假列表。
...
- 心灵驿站和班级风采中的图片会直接预览；系统只保存压缩后的展示图，减少服务器磁盘占用
- 图片读取会校验登录态与班级访问权限，不能跨班级直接打开
- 图片接口支持 `ETag`、`Last-Modified` 和私有缓存，同一张图重复打开时优先走浏览器缓存或 304
- 前端会复用已请求过的鉴权图片 Blob，并对首屏少量图片做预取；这不会降低 WebP 展示质量

常用接口：

- GET /api/class-space/overview
- GET /api/class-space/detail?className=...
- GET /api/class-space/media/wellbeing/:id
- GET /api/class-space/media/gallery/:id
...
- 模板模式会根据输出类型自动筛选兼容模型
- 若模型返回思考链，可在界面折叠查看
- 模板页只展示老师需要理解的用户说明、变量说明与模型能力标签，不展示系统提示词正文和内部格式规范
- 选科建议这种结构化场景，流式阶段只展示友好正文与思考过程，不再直接展示原始 JSON
...
- 所有删除入口都必须先确认，避免误触。
- 整班级联删除会保留账号发放批次审计；若历史操作者账号被删除，系统会自动重挂到“系统审计保留”身份，避免大批量删除时外键失败。
- 当前线上同步脚本不会上传本地 `app.db`；同时生产环境默认禁用演示种子，不会因部署或重启把本地测试数据重新灌到线上。
...
4. 若看到 `瀛﹂敓`、`鐜嬭开`、`寮犳垐` 这类典型 UTF-8/GBK 误解码文本，也会在新版本中自动进入修复链路
5. 若仍异常，再导出少量样本定位具体源文件问题

后端文本修复规则升级后会更新维护版本号。线上自动同步并重启服务后，历史数据库文本维护会在后台再执行一次，不需要手工进入服务器改库。

### 6.7 AI 思考链里看到乱码怎么办

若你在思考链里看到类似 `"鑹烘湳,璁捐"` 的内容，说明旧版本把脏文本送进了模型。

当前版本已改为：

1. 进入模型前先修复兴趣、目标、考试名称、学科名、补充信息
2. 若仍疑似乱码，不原样发送给模型
3. 直接降级为“暂无有效兴趣信息”等安全占位

因此，思考链里不应再出现“模型自己解释乱码来源”的现象。

### 6.8 AI 助手会不会把内部提示词展示给老师

不会。

当前版本已经收口为：

1. 前端只展示模板用途、变量说明、结果说明和模型能力
2. 不展示任何系统提示词正文
3. 不展示内部结构化输出规范原文

老师看到的是可理解的功能说明，而不是底层提示词。
...
3. `glm-4.7-flash`：可能偶发上游限流，若失败优先考虑重试或切换模型
