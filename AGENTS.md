...
## 2026-05-02 16:39:16 +0800

- 完成组织架构层级优化：
  - `OrgStructurePanel` 为班级块、内部人员区和状态胶囊增加专用样式类。
  - `styles.css` 调整状态胶囊居中、班级间分割线和班级内部细分线，降低“所有分割线同权重”的视觉混淆。
- 完成院校专业录取分维护闭环：
  - `public_major_requirements` 增加 `data_source`、`updated_at` 字段，并将历史种子数据标记为 `demo_seed`。
  - `dataImport.ts` 新增录取分 XLSX/CSV 导入、列表查询、手动新增、编辑、单条删除和批量删除接口，导入复用现有编码检测与中文修复链路。
  - 新增 `major-requirements-template.csv` 与 `major-requirements-template.xlsx` 模板。
  - `DataImportPanel` 新增“院校专业录取分”导入卡片和录取分数据维护区，支持筛选、编辑、新增和删除。
  - `CareerPanel` 显示录取分来源提示与来源标记，避免演示数据被误认为官方录取线。
- 文档同步：
  - 更新 `README.md`、`GUIDE.md`，补充录取分演示数据说明、官方数据导入和手动维护入口。

## 2026-05-02 16:43:46 +0800

- 继续优化组织架构年级层级：
  - `OrgStructurePanel` 为年级容器增加 `org-grade-card` 专用样式类。
  - `styles.css` 将年级之间的分隔提升为更粗的一级分隔，并增加年级区块顶部留白，避免年级分隔和班级内部细分线混淆。
